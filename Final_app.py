...
def auth_screen():
    def auth_screen():
        ...
        st.markdown("<p style='text-align:center; color:#8C8178;'>Healthcare Capacity Management System</p>", unsafe_allow_html=True)
    ...
    with col:
        ...
        if mode == "Sign Up":
            # --- THE NEW FIELDS FOR THE JUDGES ---
            name = st.text_input("Hospital Name", placeholder="e.g. Manipal")
            h_id = st.text_input("Hospital ID (CMS)", placeholder="CMSXXXXX")
            email = st.text_input("Business Official Email", placeholder="admin@gmail/hotmail.com")
            region = st.selectbox("Facility Region", ["Karnataka", "Maharashtra", "Delhi", "Tamil Nadu", "Andhra Pradesh","Goa"])
            pw = st.text_input("Create Password", type="password")
          
        if st.button("Create Account"):
                if not name or not h_id or not pw or not email:
            ...
            if st.button("Sign In"):
                ...
                if user:
                    ...
                    # Pull the new data into the session so we can display it!
                    st.session_state.email = user.get("email", "Not Provided")
                    st.session_state.region = user.get("region", "Unknown")
                    st.rerun()
                else:
                    st.error("Invalid Hospital ID or Password.")
                    st.markdown("</div>", unsafe_allow_html=True)# ── 5. Main Application Pages ─────────────────────────────────────────────────
def sidebar():
    with st.sidebar:
